	...
	if (image) {
		...
		imageService.imageUpload((path === '' ? name : path + name), base64Data).then((response) => {
			...
			customerService.getProfile(authorization).then((response) => {
				...
				if (req.body.countryId) userData['country_id'] = req.body.countryId;
				if (req.body.emailId) userData['email'] = req.body.emailId;
				if (req.body.firstName) userData['first_name'] = req.body.firstName;
				if (req.body.lastName) userData['last_name'] = req.body.lastName;
				if (req.body.phoneNumber) userData['mobile'] = req.body.phoneNumber;
				if (req.body.pincode) userData['pincode'] = req.body.pincode;
				if (req.body.zoneId) userData['zone_id'] = req.body.zoneId;
